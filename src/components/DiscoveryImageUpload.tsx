...
import { uploadDiscoveryImage } from "@/lib/supabase/discoveries";

const MAX_SIZE_BYTES = 10 * 1024 * 1024;

type DiscoveryImageUploadProps = {
  value?: string | null;
...
export default function DiscoveryImageUpload({
  value,
  onChange,
}: DiscoveryImageUploadProps) {
  const { user } = useAuth();
  ...
  const handleFile = useCallback(
    async (file: File) => {
      ...
      try {
        const { url } = await uploadDiscoveryImage(file, user.id);
        setPreview(url);
        onChange?.(url);
      } catch {
